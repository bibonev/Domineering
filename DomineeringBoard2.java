...
public class DomineeringBoard2 extends Board2<DomineeringMove> {
	...
	Board2<DomineeringMove> play(DomineeringMove move) {
		...
		if (nextPlayer() == H) {
			return new DomineeringBoard2(add(hMoves, move), vMoves, playMove(playBoard, move));
		} else {
			return new DomineeringBoard2(hMoves, add(vMoves, move), playMove(playBoard, move));
		}
	}
